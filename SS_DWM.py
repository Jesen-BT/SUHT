...
class SemiDynamicWeightedMajorityClassifier(BaseSKMObject, ClassifierMixin, MetaEstimatorMixin):
    ...
    def __init__(self, n_estimators=5, base_estimator=SemiHoeffdingTreeClassifier(),
    ...
    def ss_partial_fit(self, X):
        ...
        for i in range(row_cnt):
            for exp in self.experts:
                exp.estimator.ss_partial_fit(np.asarray([X[i]]), sample_weight=np.asarray([1.0]))
    ...
    def partial_fit(self, X, y, classes=None, sample_weight=None):
        ...
        for i in range(len(X)):
            self.fit_single_sample(
                X[i:i+1, :], y[i:i+1], self.classes, sample_weight
            )
        return self
