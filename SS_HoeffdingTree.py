...
from skmultiflow.trees.split_criterion import GiniSplitCriterion, InfoGainSplitCriterion, HellingerDistanceCriterion

from skmultiflow.trees.attribute_test import NominalAttributeMultiwayTest

from SS_node import Node
from SS_node import ActiveLearningNode
from SS_node import LearningNode
from SS_node import LearningNodeNB
from SS_node import LearningNodeNBAdaptive
from SS_node import SplitNode
from SS_node import FoundNode

from SS_node import RandomLearningNodeClassification
from SS_node import RandomLearningNodeNB
from SS_node import RandomLearningNodeNBAdaptive
from SS_node import InactiveLearningNode

from skmultiflow.rules.base_rule import Rule
from skmultiflow.utils import check_random_state


class SemiHoeffdingTreeClassifier(BaseSKMObject, ClassifierMixin):
    ...
    def init_class_size(self, classes):
        self.classes = classes
        self.class_size = np.ones(len(self.classes)) / len(self.classes)
    ...
    def ss_partial_fit(self, X, sample_weight=None, classes=None):
        ...
        for i in range(row_cnt):
            if sample_weight[i] != 0.0:
                self._ss_partial_fit(X[i], sample_weight[i])

    def _ss_partial_fit(self, X, sample_weight=None):
        try:
            found_node = self._tree_root.filter_instance_to_leaf(X, None, -1)
            leaf_node = found_node.node
            if isinstance(leaf_node, LearningNode):
                learning_node = leaf_node
                learning_node.ss_learn_from_instance(X, sample_weight, self)
        except :
            pass
    ...
    def partial_fit(self, X, y, classes=None, sample_weight=None):
        ...
        * If growth is allowed and the number of instances that the leaf has
        ...
        if y is not None:
            ...
            for i in range(row_cnt):
                self.class_size = self.class_size * self.theta
                self.class_size[y[i]] = self.class_size[y[i]] + (1 - self.theta)
        ...
        if not node.observed_class_distribution_is_pure():
            ...
            if len(best_split_suggestions) < 2:
                ...
                if (best_suggestion.merit - second_best_suggestion.merit > hoeffding_bound
                    ...
                    should_split = True
                    #分裂置信度逐渐提高，即随着树的生长来提高分类精度
                    self.split_confidence = self._split_confidence * self.alpha
...
class ARFHoeffdingTreeClassifier(SemiHoeffdingTreeClassifier):
